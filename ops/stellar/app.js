(() => {
  ...
  function deliverablesTable(rows) {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Entregable y requisito del SOW</th><th>Responsable</th><th>Vence</th><th>Estado</th><th></th></tr></thead><tbody>${rows.map((d) => `<tr><td class="deliverable-copy"><button class="table-link" data-edit-deliverable="${esc(d.id)}">${esc(d.title)}</button>${d.description?`<p>${esc(d.description)}</p>`:""}${resourceLinks(d.resource_links)}</td><td>${esc(ownerName(d.owner_id))}</td><td>${fmtDate(d.due_on)}</td><td>${status(d.status)}</td><td><button class="icon-button" data-edit-deliverable="${esc(d.id)}" aria-label="Editar ${esc(d.title)}">${icon("pencil")}</button></td></tr>`).join("")}</tbody></table></div>`;
  }
