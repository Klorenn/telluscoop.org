(() => {
  ...
  const state = {
    ...
    participantFilters: { search:"", rank:"all", country:"all", from:"", to:"" },
  };
  ...
  function participantFilterRows(rows) {
    ...
    return rows.filter((row)=>{
      ...
      return (!query||joined.includes(query))&&(f.rank==="all"||row.participant_rank===f.rank)&&(f.country==="all"||row.country===f.country)&&(!f.from||date>=f.from)&&(!f.to||date<=f.to);
    });
  }
  function rankChart(rows) { const ranks=["Explorer","Builder","Contributor","Leader"]; const max=Math.max(1,...ranks.map((rank)=>rows.filter((row)=>row.participant_rank===rank).length)); return `<div class="rank-chart" aria-label="Distribución de participantes por rango">${ranks.map((rank)=>{const count=rows.filter((row)=>row.participant_rank===rank).length;return `<div class="rank-chart-row"><span>${rankBadge({participant_rank:rank})}</span><div class="rank-chart-track"><span class="rank-chart-bar rank-bar-${rank.toLowerCase()}" style="width:${Math.round(count/max*100)}%"></span></div><strong>${count}</strong></div>`;}).join("")}</div>`; }
  function participantsView() {
    ...
    const countries=[...new Set(allRows.map((row)=>row.country).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"es"));
    const totalEvents=rows.reduce((sum,row)=>sum+Number(row.events_attended_count||0),0); const githubCount=rows.filter((row)=>row.github).length; const advanced=rows.filter((row)=>row.participant_rank!=="Explorer").length;
    ...
    return `<div class="toolbar"><div><span class="eyebrow">${esc(selectedProgram()?.name || "Toda la operación")}</span><h2>Participantes</h2></div><div class="topbar-actions">${actions}</div></div>
      <section class="participant-kpis"><article class="card participant-kpi">${icon("users")}<div><strong>${rows.length}</strong><span>${rows.length===allRows.length?"participantes":"de "+allRows.length+" participantes"}</span></div></article><article class="card participant-kpi">${icon("calendar-check")}<div><strong>${totalEvents}</strong><span>asistencias acumuladas</span></div></article><article class="card participant-kpi">${icon("trending-up")}<div><strong>${advanced}</strong><span>Builder o superior</span></div></article><article class="card participant-kpi">${icon("github")}<div><strong>${githubCount}</strong><span>con GitHub</span></div></article></section>
      <section class="participant-overview"><article class="card section-card participant-chart-card"><div class="section-head"><div><h2>Distribución por rango</h2><p>Resultados del filtro actual.</p></div></div>${rankChart(rows)}</article><article class="card section-card participant-filter-card"><div class="section-head"><div><h2>Filtrar roster</h2><p>Busca por persona, contacto, ubicación o fecha.</p></div></div><form id="participant-filter-form" class="participant-filter-grid"><div class="field filter-search"><label for="participant-search">Buscar</label><div class="filter-input-icon">${icon("search")}<input id="participant-search" name="search" value="${esc(f.search)}" placeholder="Nombre, correo, GitHub…" /></div></div><div class="field"><label for="participant-rank">Rango</label><select id="participant-rank" name="rank"><option value="all">Todos</option>${["Explorer","Builder","Contributor","Leader"].map((rank)=>`<option value="${rank}" ${f.rank===rank?"selected":""}>${rank}</option>`).join("")}</select></div><div class="field"><label for="participant-country">País</label><select id="participant-country" name="country"><option value="all">Todos</option>${countries.map((country)=>`<option value="${esc(country)}" ${f.country===country?"selected":""}>${esc(country)}</option>`).join("")}</select></div><div class="field"><label for="participant-from">Desde</label><input id="participant-from" name="from" type="date" value="${esc(f.from)}" /></div><div class="field"><label for="participant-to">Hasta</label><input id="participant-to" name="to" type="date" value="${esc(f.to)}" /></div><div class="filter-actions"><button class="button button-primary" type="submit">${icon("sliders-horizontal")} Aplicar</button><button class="button button-ghost" id="clear-participant-filters" type="button">Limpiar</button></div></form></article></section>
      <article class="card section-card"><div class="section-head"><div><h2>Roster</h2><p>${rows.length} resultados</p></div></div><div class="rank-rule">${icon("sparkles")} Builder se asigna automáticamente con 3 asistencias, o con 2 si la persona tiene GitHub. Contributor y Leader son manuales.</div><div class="participant-list">${rows.map((row)=>`<article class="participant-row"><div class="participant-main"><div class="participant-identity"><strong>${esc(row.full_name || "Sin nombre")}</strong><div class="rank-meta">${rankBadge(row)}<span>${esc(row.events_attended_count ?? 0)} eventos</span><span>${row.rank_mode === "manual" ? "Asignación manual" : "Rango automático"}</span></div></div><div class="participant-actions"><div class="participant-contacts">${participantLink("mail",row.email,`mailto:${row.email}`)}${participantLink("github",row.github,row.github?.startsWith("http") ? row.github : `https://github.com/${String(row.github||"").replace(/^@/,"")}`,true)}${participantLink("message-circle",row.discord,`https://discord.com/users/${row.discord}`,true)}${participantLink("message-circle-more",row.phone,`https://wa.me/${String(row.phone||"").replace(/\D/g,"")}`,true)}${participantLink("globe",row.personal_url,row.personal_url,true)}</div>${state.preview ? "" : `<button class="button button-secondary rank-button" data-manage-rank="${esc(row.id)}">${icon("badge-check")} Gestionar rango</button>`}</div></div>${participantDetails(row)}</article>`).join("")}</div>${rows.length ? "" : `<div class="empty">No encontramos participantes con estos filtros.</div>`}</article>`;
  }
  ...
  function wireShell() {
    ...
    document.querySelector("#clear-participant-filters")?.addEventListener("click", () => { state.participantFilters={search:"",rank:"all",country:"all",from:"",to:""}; renderShell(); });
