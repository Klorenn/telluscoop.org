(() => {
  ...
  const teamAccounts = [
    ["hola@telluscoop.org","Tellus Cooperative Admin"], ["kohcuendepau@gmail.com","Pau Koh"],
    ["bastian@telluscoop.org","Bastian Koh"], ["mishekoh@gmail.com","Mishelle"],
    ["kohcuendedani@gmail.com","Daniel"], ["alexbnjmnch@gmail.com","Alex Hernández"],
  ...
  function metricWithUpdate(metric) {
    const saved = periodUpdates().find((u) => u.metric_id === metric.id) || { actual:0, status:"not_started" };
    const isEventMetric = metric.code === "events" || /eventos calificables/i.test(metric.label || "");
    if (!isEventMetric) return { ...metric, update:saved };
    const actual = state.initiatives.filter((item) => item.period_id === state.selectedPeriod && inProgram(item) && item.type === "event" && ["submitted","accepted"].includes(item.status)).length;
    const status = actual >= Number(metric.target || 0) ? "accepted" : actual > 0 ? "in_progress" : "not_started";
    return { ...metric, update:{ ...saved, actual, status } };
  }
  ...
  function teamName(email) { return teamAccounts.find(([accountEmail]) => accountEmail === String(email || "").toLowerCase())?.[1] || email || "Sin asignar"; }
  function ownerName(idOrEmail) { return String(idOrEmail || "").includes("@") ? teamName(idOrEmail) : state.members.find((member) => member.id === idOrEmail)?.full_name || "Sin asignar"; }
  ...
  function priorityItems(metrics, next) {
    const items = metrics.filter((m) => ["at_risk","blocked"].includes(m.update.status)).slice(0,3).map((m) => ({ icon:"triangle-alert", title:m.label, detail:`${m.update.actual} de ${m.target} ${m.unit}`, type:"metric", id:m.id }));
    if (next) items.unshift({ icon:"calendar-clock", title:next.title, detail:`Vence ${fmtDate(next.due_on)}`, type:"deliverable", id:next.id });
    if (!items.length) return `<div class="empty">Sin alertas activas.</div>`;
    return items.slice(0,4).map((i) => `<button class="mini-item priority-action" data-priority-type="${i.type}" data-priority-id="${esc(i.id)}"><div class="mini-icon">${icon(i.icon)}</div><div><strong>${esc(i.title)}</strong><span>${esc(i.detail)}</span></div>${icon("arrow-right")}</button>`).join("");
  }

  function deliverablesTable(rows) {
    if (!rows.length) return `<div class="empty">${icon("inbox")}<div>No hay entregables.</div></div>`;
    return `<div class="table-wrap"><table><thead><tr><th>Entregable y requisito del SOW</th><th>Responsable</th><th>Vence</th><th>Estado</th><th></th></tr></thead><tbody>${rows.map((d) => `<tr><td class="deliverable-copy"><button class="table-link" data-edit-deliverable="${esc(d.id)}">${esc(d.title)}</button>${d.description?`<p>${esc(d.description)}</p>`:""}${resourceLinks(d.resource_links)}</td><td>${esc(ownerName(d.owner_email || d.owner_id))}</td><td>${fmtDate(d.due_on)}</td><td>${status(d.status)}</td><td><button class="icon-button" data-edit-deliverable="${esc(d.id)}" aria-label="Editar ${esc(d.title)}">${icon("pencil")}</button></td></tr>`).join("")}</tbody></table></div>`;
  }
  ...
  function resourceTypeFromUrl(url){const value=String(url||"").toLowerCase();if(value.includes("docs.google.com/spreadsheets"))return "google_sheets";if(value.includes("docs.google.com/forms"))return "form";if(value.includes("docs.google.com/presentation"))return "presentation";if(value.includes("drive.google.com")||value.includes("docs.google.com/document"))return "google_drive";if(value.includes("notion."))return "notion";if(value.includes("github.com"))return "github";return "other";}
  function resourceMeta(row){return {google_drive:["folder","Carpetas de Google Drive"],google_sheets:["sheet","Google Sheets"],notion:["notebook-tabs","Notion"],form:["clipboard-list","Formularios"],presentation:["presentation","Presentaciones"],github:["github","GitHub"],other:["link","Otros recursos"]}[row.resource_type||resourceTypeFromUrl(row.url)]||["link","Otros recursos"];}
  function resourcesView() { const rows=state.resources.filter(inProgram).sort((a,b)=>resourceMeta(a)[1].localeCompare(resourceMeta(b)[1],"es")||a.title.localeCompare(b.title,"es"));const groups=rows.reduce((all,row)=>{const key=resourceMeta(row)[1];(all[key]||=[]).push(row);return all;},{}); return `<div class="toolbar"><div><span class="eyebrow">${esc(selectedProgram()?.name || "Toda la operación")}</span><h2>Planillas y recursos</h2></div>${state.selectedProgram !== "global" ? `<button class="button button-primary" id="add-resource">${icon("plus")} Añadir enlace</button>` : ""}</div>${rows.length?Object.entries(groups).map(([label,items])=>`<section class="resource-group"><div class="section-head"><div><h2>${esc(label)}</h2><p>${items.length} ${items.length===1?"recurso":"recursos"}</p></div></div><div class="resource-grid">${items.map((row)=>{const [resourceIcon]=resourceMeta(row);return `<article class="card resource-card"><div class="resource-card-head"><span class="resource-icon">${icon(resourceIcon)}</span><span class="type-chip">${esc(programName(row.program_id))}</span></div><h3>${esc(row.title)}</h3><p>${esc(row.description || "Sin descripción")}</p><div class="resource-actions"><a class="button button-secondary" href="${esc(row.url)}" target="_blank" rel="noopener">${icon("external-link")} Abrir</a>${state.preview?"":`<button class="icon-button" data-edit-resource="${esc(row.id)}" aria-label="Editar ${esc(row.title)}">${icon("pencil")}</button>`}</div></article>`;}).join("")}</div></section>`).join(""):`<div class="empty">No hay planillas o recursos en este espacio.</div>`}`; }
  ...
  function participantsView() {
    const allRows=state.participants.filter(inProgram); const filteredRows=participantFilterRows(allRows); const f=state.participantFilters; const pageSize=30; const pageCount=Math.max(1,Math.ceil(filteredRows.length/pageSize)); state.participantPage=Math.min(state.participantPage,pageCount); const pageStart=(state.participantPage-1)*pageSize; const rows=filteredRows.slice(pageStart,pageStart+pageSize);
  ...
  function wireShell() {
    ...
    document.querySelector("#participant-filter-form")?.addEventListener("submit", (event) => { event.preventDefault(); state.participantPage=1;state.participantFilters={...state.participantFilters,...Object.fromEntries(new FormData(event.currentTarget))}; renderShell(); });
    document.querySelector("#participant-search")?.addEventListener("input", (event) => { const value=event.target.value; state.participantFilters.search=value;state.participantPage=1; clearTimeout(participantSearchTimer); participantSearchTimer=setTimeout(()=>{renderShell();const input=document.querySelector("#participant-search");if(input){input.focus();input.setSelectionRange(value.length,value.length);}},180); });
    document.querySelectorAll("#participant-filter-form select, #participant-filter-form input[type=date]").forEach((control)=>control.addEventListener("change",()=>{state.participantPage=1;state.participantFilters={...state.participantFilters,...Object.fromEntries(new FormData(document.querySelector("#participant-filter-form")))};renderShell();}));
    document.querySelector("#clear-participant-filters")?.addEventListener("click", () => { state.participantPage=1;state.participantFilters={search:"",rank:"all",city:"all",country:"all",from:"",to:""}; renderShell(); });
    ...
    document.querySelectorAll("[data-priority-type]").forEach((button) => button.addEventListener("click", () => {
      ...
      else openDeliverableModal(state.deliverables.find((row) => row.id === button.dataset.priorityId));
    }));
  ...
  async function loadLumaPicker(event) {
    ...
    document.querySelector("#apply-luma-event")?.addEventListener("click",applyLumaSelection);
  }
  ...
  async function importContactEmailsFromFile(){
    const input=document.querySelector("#contact-file"),message=document.querySelector("#contact-import-message"),file=input?.files?.[0];if(!file){message.textContent="Selecciona un archivo primero.";return;}
    try{const rows=workbookRows(await file.arrayBuffer());const emails=[...new Set(rows.flatMap((row)=>Object.values(row)).flatMap((value)=>String(value||"").split(/[\s,;]+/)).map((value)=>value.trim().toLowerCase()).filter((value)=>/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)))];const textarea=document.querySelector("#contact_emails");const existing=String(textarea.value||"").split(/\r?\n|,/).map((value)=>value.trim().toLowerCase()).filter(Boolean);textarea.value=[...new Set([...existing,...emails])].join("\n");message.textContent=emails.length?`${emails.length} correos añadidos desde ${file.name}.`:"No encontramos correos válidos en el archivo.";}catch(error){message.textContent=`No pudimos leer el archivo: ${error.message}`;}
  }

  function openProgramLeadsModal(){
    const program=selectedProgram();if(!program)return;const selected=new Set(Array.isArray(program.lead_emails)&&program.lead_emails.length?program.lead_emails:[program.lead_email].filter(Boolean));
    modal("Responsables del programa",`<form id="program-leads-form"><p>Selecciona una o más personas responsables de <strong>${esc(program.name)}</strong>.</p><div class="lead-options">${teamAccounts.map(([email,name])=>`<label class="lead-option"><input type="checkbox" name="lead_email" value="${esc(email)}" ${selected.has(email)?"checked":""}/><span><strong>${esc(name)}</strong><small>${esc(email)}</small></span></label>`).join("")}</div><div class="form-message" id="lead-message"></div><div class="modal-actions"><button type="button" class="button button-secondary" id="cancel">Cancelar</button><button class="button button-primary" type="submit">Guardar responsables</button></div></form>`);document.querySelector("#cancel").onclick=closeModal;document.querySelector("#program-leads-form").onsubmit=async(event)=>{event.preventDefault();const emails=new FormData(event.currentTarget).getAll("lead_email");if(!emails.length){document.querySelector("#lead-message").textContent="Selecciona al menos una persona.";return;}const button=event.currentTarget.querySelector("button[type=submit]");setButtonLoading(button,"Guardando…");const{error}=await supabase.from("programs").update({lead_email:emails[0],lead_emails:emails}).eq("id",program.id);await afterMutation(error,"Responsables actualizados");};
  }
  ...
  function openResourceModal(item = {}) {
    ...
    document.querySelector("#delete-resource")?.addEventListener("click",async event=>{if(!window.confirm(`¿Eliminar “${item.title}”?`))return;setButtonLoading(event.currentTarget,"Eliminando…");const{error}=await supabase.from("program_resources").delete().eq("id",item.id);await afterMutation(error,"Recurso eliminado");});
    document.querySelector("#resource-form").onsubmit = async (event) => { event.preventDefault(); if (lockedPreview()) return; const button=event.currentTarget.querySelector("button[type=submit]"); setButtonLoading(button,"Guardando…"); const values=Object.fromEntries(new FormData(event.currentTarget));const detected=resourceTypeFromUrl(values.url);const payload={ organization_id:state.organization.id, program_id:item.program_id||state.selectedProgram, resource_type:detected==="other"?values.resource_type:detected, title:values.title, url:values.url, description:values.description, created_by:state.session.user.id };const query=item.id?supabase.from("program_resources").update(payload).eq("id",item.id):supabase.from("program_resources").insert(payload);const{error}=await query;await afterMutation(error,"Recurso guardado"); };
  }
